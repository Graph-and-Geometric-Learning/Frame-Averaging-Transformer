...
def batch_evaluate_contact_map(pred_binding_site, y_contact, mask, pos_weight=1.0):
    ...
    precision = (precision * mask).sum(dim=-1) / ((pred_binding_site > 0).float().sum(dim=-1) + 1e-8)
    # positive recall
...
def run(seed, args):
    ...
    if args.save_checkpoint:
        torch.save({"model": best_model_weight.cpu()}, f"{save_path}/best.pt")

    print('Best val score: {}'.format(val_curve[best_val_epoch - 1]))
    print('Best test score: {}'.format(test_curve[best_val_epoch - 1]))

    if args.use_wandb:
        wandb.log({f'Best test score, seed {seed}': test_curve[best_val_epoch]})
...
def main(args):
    checkpoint_folder = "BindSitePred" \
                    + "-" + str(args.dataset) \
                    + "-" + str(args.seeds) \
                    + "-" + str(args.model) \
                    + "-" + str(args.lr) \
    ...
    if args.use_wandb:
        ...
        wandb.init(project="rna_affinity", name=exp_name)
